#CÓDIGO DEL JUEGO CLIENTE MUNDIALINHO DE PENALES 2026
...
pygame.mixer.init() #Mixer es el módulo encargado del sonido
...
pygame.display.set_caption("Mundialinho De Penales 2026 😎 (CLIENT)")
...
def cargar_recurso(ruta, x, y):
    ...
    #Fondos
fondo_menu = cargar_recurso("RecursosSI/Fondos/fondo_menu_cliente.jpg", 1080, 630)
...
fondo_buscando_servidor = cargar_recurso("RecursosSI/Fondos/fondo_buscando_servidor.jpg", 1080, 630)
...
mensaje_buscando_server = cargar_recurso("RecursosSI/Mensajes/mensaje_buscando_server.png", 895, 220)
mensaje_conectado_server = cargar_recurso("RecursosSI/Mensajes/mensaje_conectado_server.png", 895, 191)
mensaje_default = cargar_recurso("RecursosSI/Mensajes/mensaje_default_cliente.png", 868, 493)
...
def cerrar_conexion_online():
    global conectado, buscando_conexión, cliente
...
def cerrar_socket_actual_cliente():
...
def buscando_servidor():
    global conectado, buscando_conexión, cliente, rival_confirma_continuar,indice_j1, confirmacion_j1, pais_j1
    global escena, primer_turno, sorteo_hecho,tiempo_animacion_moneda, tiempo_vs, tiempo_animacion_penal
    global decision_j1, decision_j2,confirmacion_online_j1, confirmacion_online_j2,contador_j1, contador_j2,pateador_actual
    global penales_pateados, penales_restantes,ganador, modo_tanda_extra, resultado_penales_j1, resultado_penales_j2
    global j1_vuelve, j2_vuelve, tiempo_conexion_perdida
        ...
        if not conectado:
            try:
                ...
                if respuesta == b"CONECTADO|":
                    ...
                    print(f"Conectado con el Servidor con Host en {HOST}.")
                else:
                    cliente.close()
            ...
            try:
                ...
                if not datos:
                    raise ConnectionResetError("El servidor se desconectó")
                else:
                    ...
                    for mensaje in mensajes:
                        ...
                        if mensaje == "CONTINUAR CONFIRMADO":
                        ...
                        elif mensaje == "J1_CONFIRMA_DESCONEXION_PROPOSITO":
                            cerrar_conexion_online()
                            escena = "ganaste_default"

                                #Rival quiere volver
                        elif mensaje == "J1_VOLVER_MENU":
                            j1_vuelve = True
                
                            #Elección Países
                        elif mensaje.startswith("J1_SEL:"):
                            indice_j1 = int(mensaje.split(":")[1])

                        elif mensaje == "J1_CONFIRMA":
                            confirmacion_j1 = True
                            pais_j1 = paises[indice_j1]
                        elif mensaje == "J1_DESCONFIRMA":
                            confirmacion_j1 = False
                            pais_j1 = None
                            ...
                            #Sorteo
                        elif mensaje.startswith("RESULTADO_SORTEO:"):
                            primer_turno = int(mensaje.split(":")[1])
                            sorteo_hecho = True
                            ...
                            #Penales
                        elif mensaje == "J1_TAPA_IZQUIERDA" or mensaje == "J1_PATEA_IZQUIERDA":
                            decision_j1 = 1
                            confirmacion_online_j1 = True
                        elif mensaje == "J1_TAPA_MEDIO" or mensaje == "J1_PATEA_MEDIO":
                            decision_j1 = 2
                            confirmacion_online_j1 = True
                        elif mensaje == "J1_TAPA_DERECHA" or mensaje == "J1_PATEA_DERECHA":
                            decision_j1 = 3
                            confirmacion_online_j1 = True

                        elif mensaje.startswith("RESULTADO_PENAL:"):
                            datos_penal = mensaje.split(":")[1].split(",")

                            contador_j1 = int(datos_penal[0])
                            contador_j2 = int(datos_penal[1])

                            decision_j1 = int(datos_penal[2])
                            decision_j2 = int(datos_penal[3])

                            pateador_actual = int(datos_penal[4])

                            penales_pateados = int(datos_penal[5])
                            penales_restantes = int(datos_penal[6])

                            if datos_penal[7] == "None":
                                ganador = None
                            else:
                                ganador = int(datos_penal[7])

                            modo_tanda_extra = bool(int(datos_penal[8]))

                            resultado_penales_j1 = [x == "1" for x in datos_penal[9]]
                            resultado_penales_j2 = [x == "1" for x in datos_penal[10]]
...
while juego_abierto:
    ...
    if escena == "menu_principal":
        ...
        if confirmacion_j1 and confirmacion_j2:
            if pais_j1 == pais_j2:
                ...
                if conectado:
                    cliente.sendall(b"J2_DESCONFIRMA|")
            else:
                error_paises_iguales = False
    ...
    elif escena == "ganador" and not modo_local and conectado:
        if j1_vuelve and j2_vuelve:
            cerrar_conexion_online()
            escena = "menu_principal"
                ...
                if modo_local:
                    ...
                    if not confirmacion_j2:
                        if evento.key == K_a or evento.key == K_LEFT:
                            indice_j2 = (indice_j2 - 1) % len(paises)
                            if conectado:
                                cliente.sendall(f"J2_SEL:{indice_j2}|".encode("utf-8"))
        ...
        if evento.type == pygame.MOUSEBUTTONDOWN:
                ...
                if mensaje_volver:
                    if boton_volver_menu["rect"].collidepoint(evento.pos):
                        ...
                        if not modo_local and conectado:
                            cliente.sendall(b"J2_CONFIRMA_DESCONEXION_PROPOSITO|")
                            cerrar_conexion_online()
            ...
            if escena == "menu_principal":
                ...
                if boton_local_menu["rect"].collidepoint(evento.pos) and not mensaje_volver:
                    ...
                    print("Jugador 2 [Cliente] Buscando Servidor...")
                    ...
                    if not conectado and not buscando_conexión:
                        ...
                        hilo_red = threading.Thread(target=buscando_servidor, daemon=True)
                        hilo_red.start()
            ...
            #SELECCIÓN EQUIPOS
                ...
                if modo_local:
                    ...
                    if boton_confirmar_seleccion2["rect"].collidepoint(evento.pos):
                        sonido_click_decision.play()
                        if not confirmacion_j2:
                            pais_j2 = paises[indice_j2]
                            confirmacion_j2 = True
                            if conectado:
                                cliente.sendall(b"J2_CONFIRMA|")
                        else:
                            pais_j2 = None
                            confirmacion_j2 = False
                            if conectado:
            ...
            #SORTEO
                ...
                if sorteo_hecho and boton_continuar_sorteo["rect"].collidepoint(evento.pos):
                    ...
                    if not modo_local and conectado:
                        cliente.sendall(b"INICIAR_PARTIDO|")
            ...
            #J2 TAPA
                ...
                elif not modo_local  and decision_j2 is not None:
                    if decision_j2 == 1: cliente.sendall(b"J2_TAPA_IZQUIERDA|")
                    elif decision_j2 == 2: cliente.sendall(b"J2_TAPA_MEDIO|")
                    elif decision_j2 == 3: cliente.sendall(b"J2_TAPA_DERECHA|")
            ...
            #J2 PATEA
                ...
                if modo_local and decision_j2 is not None:
                    ...
                    if decision_j2 == 1: cliente.sendall(b"J2_PATEA_IZQUIERDA|")
                    elif decision_j2 == 2: cliente.sendall(b"J2_PATEA_MEDIO|")
                    elif decision_j2 == 3: cliente.sendall(b"J2_PATEA_DERECHA|")
                ...
                if boton_volver_conexion["rect"].collidepoint(evento.pos):
                    ...
                    if conectado:
                        if not confirmado_continuar:
                            ...
                            cliente.sendall(b"CONTINUAR CONFIRMADO")
                        else:
                            confirmado_continuar = False
                            cliente.sendall(b"CONTINUAR DESCONFIRMADO")
...
#BUSCANDO CONEXIÓN
        ...
        pantalla.blit(fondo_buscando_servidor, (0,0))

        dibujar_boton_img(pantalla, boton_volver_conexion, posicion_mouse)

        if not conectado:
            pantalla.blit(mensaje_buscando_server, (93, 122))
        else:
            pantalla.blit(mensaje_conectado_server, (93, 151))
...
#J2_PATEA
        ...
        if modo_local:
            ...
            if decision_j2 is not None and not confirmacion_online_j1:
                texto = fuente_3.render(f"ESPERANDO LA DECISIÓN DE {pais_j1}", True, (255,255,255))
